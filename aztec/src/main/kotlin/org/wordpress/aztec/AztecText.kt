...
class AztecText : EditText, TextWatcher {
    ...
    fun makeBlockSpan(textFormat: TextFormat): LeadingMarginSpan {
        when (textFormat) {
    ...
    private fun applyBlockStyle(listType: TextFormat, start: Int = selectionStart, end: Int = selectionEnd) {
        if (start != end) {
    ...
    private fun removeBlockStyle(start: Int = selectionStart, end: Int = selectionEnd) {
        ...
        if (spanPrecedesLine) {
            editableText.setSpan(makeBlockSpan(span.javaClass), spanStart, startOfLine - 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        }
    ...
    fun markBlockElementLineBreaks() {
        text.getSpans(0, text.length, AztecBlockSpan::class.java).forEach {
            ...
            if (it is AztecListSpan && spanEnd - 1 > spanStart && text.length > spanEnd
                ...
                text.setSpan(BlockElementLinebreak(), spanEnd - 2, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
            }
